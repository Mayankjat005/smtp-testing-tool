...
async function callApi(action, data = {}) {
    try {
        ...
        const response = await fetch('api_new.php', {
            method: 'POST',
            body: formData
        });
...
document.getElementById('menuFooterForm').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        showMessage('menuFooterMessage', 'Invalid menu JSON format', true);
    }
});
...
document.getElementById('addPostBtn').addEventListener('click', () => {
    ...
    document.getElementById('postTime').value = new Date().toISOString().slice(0, 16);
    document.getElementById('blogPostFormContainer').classList.remove('hidden');
});
...
document.getElementById('postName').addEventListener('input', function() {
    if (!document.getElementById('postId').value) { // Only for new posts
